...
function openFacR(i0, isOpen, contributors, cities, status)
	isOpen[i0] = true
...
function genArrays(groups, out)
	...
	for j in 1:n 
		for i in 1:m
			A[n+(j-1)*m+i,(j-1)*m+i] = 1

			A[n+(j-1)*m+i,m*n+n+i] = -1
		end
	end

	#sum_{j in C_g}z_j <= l_g
	for g in 1:size(out)[1]
		for j in 1:n
			if groups[j] == g
				A[n+n*m+g,n*m+j] = 1
			end
...
function genTruncArrays(close, num, groups, out)
	...
	for j in 1:n 
		for iind in 1:size(close[j])[1]
			...
			A[n+soFar,num+n+close[j][iind]] = -1
		end
	end
...
function solveTruncLP(groups, out, q)
	...
	A, b, f = genTruncArrays(close, num, groups, out)
	x = Variable(num+m+n)
	...
	return x.value, OPTval, close, num
end
...
function round(xzy)
	...
	return status, facilities, loss
end

# ╔═╡ abbc8843-4815-4856-a254-29e84aac6c44
function roundTrunc(xzy, num)
	status = Dict()
	facilities = Dict()
	facs = []

	for i in 1:m
		if xzy[num+n+i] >= 1 - eps
			facilities[i] = []
			push!(facs, i)
		end
	end
...
function openFac(i0, isOpen, contributors, cities, status, freeze, groups, t, covG)
	isOpen[i0] = true
	for j in contributors[i0]
		if status[j] == 2
			status[j] = 3
...
function event(i0, j0, isOpen, contributors, cities, status, freeze, covG, groups)
	t = dist[j0][i0]
	if status[j0] == 2 && isOpen[i0]
		status[j0] = 3
		push!(cities[i0], j0)
	...
	for i in 1:m
		if !isOpen[i]
			...
			if cont >= costs[i]
				openFac(i, isOpen, contributors, cities, status, freeze, groups, t, covG)
			end
		end
	end
...
function increment(tl, covG, groups)
	...
	for e in tl
		done = event(e[2],e[1], isOpen, contributors, cities, status, freeze, covG, groups)
		if done
			break
		end
	end
